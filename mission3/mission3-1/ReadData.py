...
def read1(path):
    ...
    for i in range(len(datas)/52):
        ...
        if label_seq.count(label_seq[3]) < 52:
            continue
        else:
            label = sum(label_seq)/52
        if label == 0:
            continue
        else:
            label = label - 1
        ...
        ave_x = sum(x_seq)/52.0
        ave_y = sum(y_seq)/52.0
        ave_z = sum(z_seq)/52.0
        ...
        var_x = sqr_x/52 - ave_x**2
        var_y = sqr_y/52 - ave_y**2
        var_z = sqr_z/52 - ave_z**2
        ave_r = math.sqrt(ave_x**2+ave_y**2+ave_z**2)
        ...
        data_seq.append(math.sqrt(var_x+var_y+var_z))
